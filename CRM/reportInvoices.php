...
if (isset($_GET['search']) && $_GET['search']==1) {
    ...
    $sql = "SELECT C.id, I.id AS iid, I.idate, I.amount, I.vat, I.icode, "
            ...
            . "C.eponimia) AS cname, C.afm, C.doy, C.address, IC.code AS scode, I.myDataTransfered, I.myDataMark, accesstoken  "
            ...
            <div class="main">
                ...
                if ($sql!="") {
                    $dg = new datagrid("dg", $db1, $sql, 
                        array("iid","idate", "scode", "icode", "cname", "afm", "doy", "address", "amount", "vat", "myDataTransfered", "myDataMark"), 
                        array("ID","ΗΜΕΡ.", "ΣΕΙΡΑ", "ΚΩΔ", "ΕΠΩΝΥΜΙΑ", "ΑΦΜ", "ΔΟΥ", "ΔΝΣΗ", "ΚΑΘ.ΑΞΙΑ", "ΦΠΑ", "MYDATA", "MARK"), 
                        $ltoken);
                    $dg->set_colsFormat(array("","DATE", "", "", "", "", "", "", "CURRENCY", "CURRENCY", "", "CHECK", ""));
                    ...
                    //     $link = "tcpdf/examples/invoicepdf2.php?id=$invoice_id&accesstoken=$accesstoken";
                    //     $record['myDataMark'] = "<a target=\"_blank\" href=\"$link\">" . $record['myDataMark'] . "</a>";
                    // }
                    for ($i=0; $i < count($rs); $i++) { 
                        $invoice_id = $rs[$i]['iid'];
                        $accesstoken = $rs[$i]['accesstoken'];
                        $link = "tcpdf/examples/invoicepdf2.php?id=$invoice_id&accesstoken=$accesstoken";
                        $rs[$i]['iid'] = "<a target=\"_blank\" href=\"$link\">" . $rs[$i]['iid'] . "</a>";
                    }
                    ...
                    $total_vat = 0;
                    foreach ($rs as $record) {
                        $total_val += $record['amount'];
                        $total_vat += $record['vat'];
                    }

                    echo "<br/><h2 style=\"margin-left:1em\">ΣΥΝΟΛΟ: ". count($dg->get_rs()). " ΑΞΙΑ: ". $total_val. " ΦΠΑ: ". $total_vat. "</h2>";
                ...
                <div style="padding:20px">
                    <a class="button" onclick="window.print();">ΕΚΤΥΠΩΣΗ</a>
                </div>
                <?php } ?>
