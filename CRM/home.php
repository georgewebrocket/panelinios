...
if (($_SESSION['user_profile']==1)) {
	...
	$criteriaBlockAccess = $blockstatusnegative==1? " status = 4 ": "";
        ...
        $agentLimitedSearch = func::vlookup("keyvalue", "SETTINGS", 
                "keycode='AGENTS-LIMITED-SEARCH'", $db1);
        //config::$agents_limitsearch = $agentLimitedSearch==1? TRUE: FALSE;
        $agentLimitedSearch = $agentLimitedSearch==1? TRUE: FALSE;
...
if (isset($_REQUEST['newcalls']) && $_REQUEST['newcalls']==1) {
    ...
    $sql = "SELECT COMPANIES.id, companyname, '' AS online_url, CONCAT_WS(', ', phone1, phone2, mobilephone) AS phones, "
        ...
        . "'' AS user2, '' AS recalldate2, '' AS recalltime2, commstatus, for_renewal , mark " 
        . " FROM COMPANIES WHERE active=1 AND (commstatus IS NULL OR commstatus='') AND id IN (SELECT companyid FROM COMPANIES_STATUS WHERE userid=$userid AND (CAST(recalldate AS UNSIGNED))<=$recalldate AND status=3) ORDER BY mark DESC ";
...
if (isset($_REQUEST['BtnSearch']) && $_REQUEST['BtnSearch']=="SEARCH") {
    ...
    if (substr($s_phone, 0, 3)=='+30') {
        $s_phone = substr($s_phone, 3 );
    }
    ...
    if (($_SESSION['user_profile']>1) || !$agentLimitedSearch) {
    ...
    elseif ($_REQUEST['s-catalogue-id']!="") {
        ...
        $catalogueid = func::getCatalogueidFromPid($_REQUEST['s-catalogue-id']);
        $sql .= " AND (catalogueid=$catalogueid)";        
    }
...
//    } 
    ...
    if ($_REQUEST['s-phone']!="") {
        ...
        $criteria .= $l->l("phone")."=".$s_phone."/";
        
        $sql .= " AND id IN (SELECT company_id FROM PHONES WHERE phone='$s_phone') "; 
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['cCategory']!=0) {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['cReference']!=0) {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['cArea']!=0) {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['c_city_id']!=0) {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['cCompanyType']!=0) {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['cStatus']!=0) {
    ...
    if ($_SESSION['user_profile']>1 && $criteriaBlockAccess!="" && $_REQUEST['c_companystatus']!=3) {
        $sql .= " AND (id NOT IN (SELECT companyid FROM COMPANIES_STATUS WHERE status=4))";
    }
    
    
    
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['txtDeliveryDateFrom']!="") {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['txtDeliveryDateTo']!="") {
    ...
    //$t_voucherid
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['t_voucherid']!="") {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['txtExpireDate']!="") {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['t_domain_expires']!="") {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['cOnlineStatus']>=0) {        
        $criteria .= $l->l("ON-LINE")."=".$_REQUEST['cOnlineStatus']."/";
        $sql .= " AND (onlinestatus=".$_REQUEST['cOnlineStatus'].")";
    }
    
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['c_hasdomain']>=0) {        
        $criteria .= "Domain = ".$_REQUEST['c_hasdomain']."/";
        $sql .= " AND (domain = ".$_REQUEST['c_hasdomain'].")";
    }
    
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['c_courier']>0) {
    ...
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['c_hasAfm']>0) {
    ...
    $sql = "SELECT COMPANIES.id, companyname, '' AS online_url, CONCAT_WS(', ', phone1, phone2, mobilephone) AS phones, "
            . "status, COMPANIES.user, recalldate, recalltime, area, basiccategory, subcategory, "
            . "'' AS LASTACTIONTIME, lastactiondate, active, COMPANIES.id AS id2, '' AS status2, "
            . "'' AS user2, '' AS recalldate2, '' AS recalltime2, commstatus, for_renewal , mark " . $sql;
    
    if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $s_sort!="") {
        $sql .= " ORDER BY $s_sort, id";
    }
    elseif (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['c_companystatus'] == 4) {
        $sql .= " ORDER BY lastactiondate, id ";
        $s_sort = "lastactiondate";
    }
    elseif (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['c_companystatus'] == 3) {
...
for ($i=0;$i<count($rs);$i++) {
    $arPhones = explode(",", $rs[$i]['phones']);
    $phones = "";
    if ($arPhones[0]!="") {
        for ($p = 0; $p < count($arPhones); $p++) {
            $tel = trim($arPhones[$p]);
            $phones .= "<a class=\"tel\" title=\"$tel\" href=\"tel:$tel\">$tel <span class=\"fa fa-phone-square\"></span> </a>";    
        }                        
    }
    $rs[$i]['phones'] = $phones;

    $rs[$i]['mark'] = $rs[$i]['mark']==1? "<span style=\"font-size:40px;color:#f0f\">*</span>": "";
}
...
<body>
    ...
    <div class="main" style="margin-left: 220px;">
        ...
        <div class="col-12">
            ...
            <div class="col-3 col-md-12"><h2 style="margin-left:1em">Αναζήτηση πελατών</h2></div>

            <div class="col-9 col-md-12" style="text-align:right">
                <a target="_blank" class="button" href="editcompany.php?id=0"><?php echo $l->l("add-new") ?></a>&nbsp;
                ...
                <a onclick="SelectAll()" class="button" href="#">Επιλογή / Αποεπιλογή όλων</a>&nbsp;
            ...
            <div class="form-container">
                ...
                <form action="home.php?search=1" method="GET" style="width:100%">
                    <div class="col-6 col-md-12" style="padding:10px">
...
<!--                        <div class="col-4"><?php echo $l->l("company-name") ?></div>
                    ...
                    <div class="col-6 col-md-12" style="padding:10px">
                        ...
                        <?php
                        $cUser = new comboBox("cUser", $db1, "SELECT id, fullname FROM USERS WHERE active=1 ", 
...
<!--                        <div class="col-4">On-Line</div>
                    ...
                    <?php
                    $plusDays = 1;
                    if (date("N", strtotime("+1 day"))==6) {$plusDays = 3;}
                    if (date("N", strtotime("+1 day"))==7) {$plusDays = 2;}
                    $nextDay = date("d/m/Y", strtotime("+$plusDays day"));
                    
                    $linkPrintNextDay = "home.php?c_companystatus=5&txtDeliveryDateTo=$nextDay&BtnSearch=SEARCH&cOnlineStatus=-1&c_hasdomain=-1&t_recordsperpage=30";
...
EOT;
                    ...
                    if ($_SESSION['user_profile']>1) {
                        ...
                        &nbsp;
                        <a href="$linkPrintNextDay">
                        <input type="button" value="ΕΚΤΥΠ. ΕΠΟΜ. ΜΕΡΑΣ" />
                        </a>
EOT;
                    }
                    ?>
            ...
            <?php
                ...
                if (($_SESSION['user_profile']>1 || !$agentLimitedSearch) && $_REQUEST['c_companystatus']==4) { //arnitikos
                    for ($i=0;$i<count($rs);$i++) {
                ...
                $gridCompanies = new datagrid("gridCompanies", $db1, 
                        "", 
                        array("id2","companyname", "status2", "recalldate2","recalltime2", "phones","area","basiccategory","status", "mark"), 
    ...
    <script>
        $(function() {
            $("#gridCompanies").tableExport({formats: ["xlsx","xls", "csv"]});
