...
for ($i=0;$i<count($rs);$i++) {
    ...
    if ($i==0) {
        ...
        for ($k = 0; $k < count($keys); $k++) {
            if ($fields!="") {
                if (in_array($keys[$k], $arFields)) {
                    $objPHPExcel->getActiveSheet()->setCellValue(c($col+1).'1', $keys[$k]);
                    $col++;
                }
            }
            else {
    ...
    for ($k = 0; $k < count($keys); $k++) {
        ...
        if ($fields!="") {
            if (in_array($keys[$k], $arFields)) {
                $row = $i + 2;
                $objPHPExcel->getActiveSheet()->setCellValue(c($col+1).$row, $record[$keys[$k]]);
                $col++;
            }
        }
        else {
