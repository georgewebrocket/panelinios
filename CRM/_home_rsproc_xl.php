...
$homesql = str_replace(
    ...
    "SELECT COMPANIES.id, companyname, contactperson, CONCAT_WS(', ', phone1, phone2, mobilephone) AS phones, email, city_id, profession,   
    ...
    '' AS recalldate2, '' AS recalltime2, commstatus, for_renewal, online_url  ", 
    $homesql);

$homesql = str_replace("SLCT", "SELECT", $homesql);
//echo $homesql;
$rs = $db1->getRS($homesql);

$fields = "id,companyname,contactperson,email,phones,city_id,profession,online_url,status2,user2,recalldate2,recalltime2";
...
$rsProfessions = $db1->getRS("SELECT * FROM PROFESSIONS");
...
for ($i=0;$i<count($rs);$i++) {
    ...
    $rs[$i]['profession'] = func::vlookupRS("description", $rsProfessions, $rs[$i]['profession']);
